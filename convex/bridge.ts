...
import { ActionCtx, action, httpAction, internalAction, internalMutation, internalQuery } from "./_generated/server";
...
/**
 * Baixa + decifra a mídia de uma mensagem inbound pelo wuzapi e guarda como
 * anexo. Núcleo ÚNICO dos dois ingests (1:1 e grupo): as defesas da v0.53
 * (allowlist de mimetype, quota da org, teto de 25 MB, descriptor sem
 * `MediaKey` no banco) valem igual para os dois.
 *
 * Nunca lança: qualquer falha anota o motivo em `metadata` e devolve
 * `undefined`. Derrubar a mensagem por causa da mídia perderia o texto/legenda
 * que o contato escreveu — a mensagem entra sem o anexo, com a nota.
 */
async function downloadInboundMedia(
  ...
  const { config, media, metadata } = args;
...
export const webhookReceive = httpAction(async (ctx, request) => {
  ...
  if (parsed.kind === "message") {
    ...
    // Mesma forma do 1:1: a mídia precisa de uma action (download + storage).
    await ctx.scheduler.runAfter(0, internal.bridge.internalIngestGroupMessage, {
      configId: config._id,
      message: parsed.message,
    });
  } else if (parsed.kind === "receipt") {
    if (parsed.receipt.chatJid) {
      // Recibo de GRUPO: chega um por membro. `readBy` acumula quem leu e o
      // status só sobe no primeiro de cada tipo (nunca rebaixa).
      await ctx.runMutation(internal.conversations.internalApplyGroupReceipt, {
        organizationId: config.organizationId,
        externalIds: parsed.receipt.externalIds,
        status: parsed.receipt.status,
        readerJid: parsed.receipt.readerJid,
        at: Date.now(),
      });
    } else {
    ...
    // Mesma mutation do 1:1 — o "autor" da reação é a chave do MEMBRO (LID
    // quando existe). Alvo desconhecido (grupo não monitorado) é no-op.
    await ctx.runMutation(internal.conversations.internalApplyReaction, {
      ...
      sender: parsed.reaction.senderLid ?? parsed.reaction.senderPhone ?? "contact",
      senderName: parsed.reaction.senderName,
      at: parsed.reaction.timestamp,
    });
  } else if (parsed.kind === "group_info") {
    await ctx.runMutation(internal.groupChats.internalApplyGroupInfoEvent, {
      channelConfigId: config._id,
      jid: parsed.info.jid,
      actorJid: parsed.info.actorJid,
      at: parsed.info.timestamp,
      name: parsed.info.name,
      topic: parsed.info.topic,
      isLocked: parsed.info.isLocked,
      isAnnounce: parsed.info.isAnnounce,
      join: parsed.info.join,
      leave: parsed.info.leave,
      promote: parsed.info.promote,
      demote: parsed.info.demote,
    });
  } else if (parsed.kind === "joined_group") {
    // Entramos num grupo: CADASTRA com `monitored:false` (D4) e notifica quem
    // administra o canal. Quem decide acompanhar é uma pessoa.
    const group = parseGroupInfoStruct(parsed.joined.groupInfoRaw);
    if (group) {
      await ctx.runMutation(internal.groupChats.internalApplyJoinedGroup, {
        channelConfigId: config._id,
        group,
        reason: parsed.joined.reason,
        at: Date.now(),
      });
    }
  } else if (parsed.kind === "group_presence") {
    await ctx.runMutation(internal.conversations.internalSetGroupPresence, {
      channelConfigId: config._id,
  ...
  // parsed.kind === "ignored" (reação nossa, presence do self, unrecognized) → no-op
...
// ─────────────────────────────────────────────────────────────────────────────
// Ingest de GRUPO (v0.57)
// ─────────────────────────────────────────────────────────────────────────────

const parsedGroupMessageValidator = v.object({
  chatJid: v.string(),
  externalId: v.string(),
  fromMe: v.boolean(),
  ...
  quote: v.optional(v.object({ stanzaId: v.string(), participant: v.optional(v.string()) })),
  metadata: v.optional(v.record(v.string(), v.any())),
});
 ...
 *
 * Porta de entrada com opt-in duplo (D4): o grupo precisa estar CADASTRADO
 * neste canal e MARCADO como acompanhado. Grupo desconhecido é descartado sem
 * deixar rastro — o número do cliente está em grupo de família e de escola, e
 * ingerir tudo seria vazamento, não recurso. Grupo conhecido mas não
 * acompanhado só tem a atividade carimbada (nenhum conteúdo é gravado).
 */
export const internalIngestGroupMessage = internalAction({
  args: {
    configId: v.id("channelConfigs"),
    message: parsedGroupMessageValidator,
  },
  returns: v.null(),
  handler: async (ctx, args): Promise<null> => {
    const config = await ctx.runQuery(internal.channelConfigs.internalGetConfig, {
    ...
    if (!config || config.status !== "active") return null;
    // Interruptor do NÚMERO: com grupos desligados o CRM não ingere nada, nem
    // de grupo que já esteja cadastrado de uma configuração anterior.
    if (config.bridgeGroupsEnabled !== true) return null;

    // Numa sala o `Chat` é o grupo e o `Info.Sender`/`Info.SenderAlt` de uma
    // mensagem `fromMe` somos NÓS: esta é a única fonte de identidade própria
    // num gateway self-hosted, onde `/session/status` devolve `jid: ""` (medido
    // em 3 instâncias) e `/admin/users` exige o admin token do gateway
    // GERENCIADO. Sem ela três regras nascem mortas ao mesmo tempo: a menção à
    // IA nunca casa, o `GroupInfo` nunca detecta que NÓS fomos removidos, e o
    // público de membros de campanha manda DM para o próprio número (review de
    // correção nº 22). Roda antes do filtro de "acompanhado" de propósito — a
    // identidade do número não depende de qual sala alguém marcou — e só quando
    // o campo está vazio, então não custa nada no caso comum.
    if (
      args.message.fromMe &&
      ((config.bridgeLid === undefined && args.message.senderLid) ||
        (config.bridgePhone === undefined && args.message.senderPhone))
    ) {
      await ctx.runMutation(internal.channelConfigs.internalLearnBridgeIdentity, {
        configId: args.configId,
        ...(args.message.senderLid ? { lid: args.message.senderLid } : {}),
        ...(args.message.senderPhone ? { phone: args.message.senderPhone } : {}),
      });
    }

    const target = await ctx.runQuery(internal.conversations.internalGetGroupIngestTarget, {
      channelConfigId: args.configId,
      jid: args.message.chatJid,
    });
    if (!target) return null; // grupo desconhecido — D4

    if (!target.monitored) {
      // Não acompanhado: só atividade + nome do membro, nunca o conteúdo.
      await ctx.runMutation(internal.conversations.internalTouchGroupActivity, {
        groupChatId: target.groupChatId,
        at: args.message.timestamp,
        senderLid: args.message.senderLid,
        senderPhone: args.message.senderPhone,
        senderName: args.message.senderName,
      });
      return null;
    }

    // Idempotência antecipada: o gateway reentrega no retry.
    const existing = await ctx.runQuery(internal.conversations.internalGetMessageByExternalId, {
      organizationId: config.organizationId,
      externalId: args.message.externalId,
    });
    ...
    // Quote: resolve o id local da mensagem citada (quando ela já está no CRM).
    if (args.message.quote?.stanzaId) {
      metadata.quoted = {
        externalId: args.message.quote.stanzaId,
        ...(args.message.quote.participant
          ? { participant: args.message.quote.participant }
          : {}),
      };
      const quotedTarget = await ctx.runQuery(
        internal.conversations.internalGetMessageByExternalId,
        { organizationId: config.organizationId, externalId: args.message.quote.stanzaId }
      );
    ...
    if (args.message.fromMe) {
      // Saiu do nosso número: eco do CRM (morre na idempotência) ou alguém
      // digitando no app do celular dentro do grupo.
      await ctx.runMutation(internal.conversations.internalReceiveGroupDeviceMessage, common);
      return null;
    }

    await ctx.runMutation(internal.conversations.internalReceiveGroupMessage, {
      ...common,
      senderLid: args.message.senderLid,
      senderPhone: args.message.senderPhone,
      senderName: args.message.senderName,
